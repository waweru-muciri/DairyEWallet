...
const createDocumentDefinition = (reportTitle, ...contentParts) => {
    const baseDocDefinition = {
        ...
        info: {
            ...
            author: 'Records Services',
            subject: `${reportTitle}`,
            keywords: `${reportTitle}`,
        },
        footer: (currentPage, pageCount) => {
            return {
                text: `Records : Page ${currentPage.toString()} of ${pageCount.toString()}`,
                alignment: 'center',
                fontSize: 7
            }
        },
...
export function exportPropertyStatementDataToXSL(title, subject, headCells, dataRows, fileName) {
    const headCellsToPrint = [title.includes("Income") ? "income_type" : "product_type", ...headCells]
    const dataToExport = [];
    dataRows.forEach((row) => {
        const tableRow = {};
        headCellsToPrint.forEach((headCell) => {
