    ...
    <Formik
      ...
      onSubmit={async (values, { resetForm, setStatus }) => {
        try {
          ...
          //only record an product when a new management fee record is added
          if (!values.id) {
            const managementFeesProduct = {
              type: 'management_fees',
              amount: values.fees_amount,
              property_id: values.property_id,
              product_date: values.collection_date,
              product_notes: values.management_fees_notes,
            };
            await handleItemSubmit(managementFeesProduct, "products")
          }
          resetForm({});
          if (values.id) {
            history.goBack();
